...
?>
<?php  Capsule::PartialRender ('div', ['data-user-list' => $scope->users], Capsule::CreateElement ('h1', [], $scope->title), Capsule::Yield (null, []), Capsule::CreateElement ('Mock', [])); ?>
<?php }); ?>
...
<?php  Capsule::PartialRender ('ul', [], function ($args, CapsuleScopeContext $scope) {$ref9228010139erence = $this->names;
	if (isset ($ref9228010139erence) && is_array ($ref9228010139erence)) {
	for ($i = 0; $i < count ($ref9228010139erence); $i++) {
		$ref9228010139 = $ref9228010139erence[$i];
		if (in_array (strtolower(gettype($ref9228010139)), ['array', 'object'])) {
			$ref9228010139 = \Saml::Array2Object($ref9228010139);
			$ref9228010139_props = array_keys ((array)($ref9228010139));
			if (is_object ($ref9228010139) && in_array ('Sammy\Packs\Sami\Base\ILeanable', class_implements (get_class ($ref9228010139)))) {
				$ref9228010139_props = array_keys ((array)($ref9228010139->lean()));
			}
			foreach ($ref9228010139_props as $key) {
				if (is_right_var_name($key)) { $scope->$key = is_object ($ref9228010139) ? $ref9228010139->$key : $ref9228010139[$key]; }
			}
		}

       

 Capsule::PartialRender ('li', [], $scope->name);
